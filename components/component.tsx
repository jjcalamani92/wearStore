...
export const Button: FC<Button> = ({ content, click }) => {
  return (
    <button className="w-full px-4 py-2 font-medium tracking-wide text-white capitalize transition-colors duration-200 transform bg-red-500 rounded-md hover:bg-red-600 focus:outline-none focus:bg-red-600" onClick={() => click()}>
      {content}
    </button>
...
export const CardSite: FC<CardSite> = ({ data, url }) => {
  const { imageSrc, name, href, _id } = data
  ...
  return (
    <div  className="shadow-2xl p-2 ">
      ...
      <Button content='eliminar' click={() => onDeleteData(_id)} />
    </div>
  )
}
